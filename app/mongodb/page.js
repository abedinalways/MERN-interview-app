...
export default function MongodbPage() {
  ...
  const [filteredQuestions, setFilteredQuestions] = useState(mongodbData[activeLevel]);
  const [completedQuestions, setCompletedQuestions] = useState(new Set());

  useEffect(() => {
    const saved = localStorage.getItem(`progress-${'MongoDB'}`);
    if (saved) {
  ...
  const handleSearch = (term) => {
    setSearchTerm(term);
    if (!term) {
      setFilteredQuestions(mongodbData[activeLevel]);
      return;
    }
    const filtered = mongodbData[activeLevel].filter(
      (item) =>
  ...
  const handleLevelChange = (level) => {
    ...
    setSearchTerm('');
    setFilteredQuestions(mongodbData[level]);
  };
  ...
  const toggleQuestion = (questionId) => {
    ...
    localStorage.setItem(`progress-${'MongoDB'}`, JSON.stringify([...newCompleted]));
  };

  const totalQuestions =
    (mongodbData.basic?.length || 0) +
    (mongodbData.intermediate?.length || 0) +
    (mongodbData.advanced?.length || 0);
