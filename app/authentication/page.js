...
const QuestionCard = ({ question, answer, index, questionId, toggleQuestion, isCompleted }) => {
...
export default function AuthenticationPage() {
  ...
  const [filteredQuestions, setFilteredQuestions] = useState(authenticationData[activeLevel]);
  const [completedQuestions, setCompletedQuestions] = useState(new Set());

  useEffect(() => {
    const saved = localStorage.getItem(`progress-${'Authentication'}`);
    if (saved) {
  ...
  const handleSearch = (term) => {
    setSearchTerm(term);
    if (!term) {
      setFilteredQuestions(authenticationData[activeLevel]);
      return;
    }
    const filtered = authenticationData[activeLevel].filter(
      (item) =>
  ...
  const handleLevelChange = (level) => {
    ...
    setSearchTerm('');
    setFilteredQuestions(authenticationData[level]);
  };
  ...
  const toggleQuestion = (questionId) => {
    ...
    localStorage.setItem(`progress-${'Authentication'}`, JSON.stringify([...newCompleted]));
  };

  const totalQuestions =
    (authenticationData.basic?.length || 0) +
    (authenticationData.intermediate?.length || 0) +
    (authenticationData.advanced?.length || 0);
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      <div className="container mx-auto px-4 py-8">
        ...
        >
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-green-400 to-blue-600 bg-clip-text text-transparent">
              {authenticationData.title}
            </h1>
          </div>
          <h2 className="text-2xl md:text-3xl font-semibold text-gray-300 mb-4">
            {authenticationData.titleB}
          </h2>
          <p className="text-xl text-gray-400 max-w-2xl mx-auto">
            {authenticationData.description}
          </p>
        </motion.div>
        ...
        >
          <LevelTabs activeLevel={activeLevel} onLevelChange={handleLevelChange} />
