...
export default function ReduxPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('progress-Redux');
    if (saved) {
  ...
  const handleSearch = term => {
    ...
    const questions = reduxData[activeLevel] || [];
    if (!term.trim()) {
  ...
  // Change level (basic/intermediate/advanced)
  const handleLevelChange = level => {
    setActiveLevel(level);
    setSearchTerm('');
    setFilteredQuestions(reduxData[level] || []);
  };

  // Toggle completed state of a question
  const toggleQuestion = questionId => {
    ...
    localStorage.setItem('progress-Redux', JSON.stringify([...newCompleted]));
  };

  // Total questions count
  const totalQuestions =
    (reduxData.basic?.length || 0) +
    (reduxData.intermediate?.length || 0) +
    (reduxData.advanced?.length || 0);
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      <div className="container mx-auto px-4 py-8">
        ...
        {/* Level Tabs + Search + Progress Tracker */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
