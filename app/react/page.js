...
export default function ReactPage() {
  ...
  const [filteredQuestions, setFilteredQuestions] = useState(
    reactData[activeLevel]
  );
  const [completedQuestions, setCompletedQuestions] = useState(new Set());

  useEffect(() => {
    const saved = localStorage.getItem(`progress-${'react'}`);
    if (saved) {
  ...
  const handleSearch = term => {
    setSearchTerm(term);
    if (!term) {
      setFilteredQuestions(reactData[activeLevel]);
      return;
    }
    const filtered = reactData[activeLevel].filter(
      item =>
  ...
  const handleLevelChange = level => {
    ...
    setSearchTerm('');
    setFilteredQuestions(reactData[level]);
  };
  ...
  const totalQuestions =
    (reactData.basic?.length || 0) +
    (reactData.intermediate?.length || 0) +
    (reactData.advanced?.length || 0);
