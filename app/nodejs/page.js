...
import nodejsData from '../data/nodejsData';
...
export default function NodejsPage() {
  ...
  const [filteredQuestions, setFilteredQuestions] = useState(nodejsData[activeLevel]);
  const [completedQuestions, setCompletedQuestions] = useState(new Set());

  useEffect(() => {
    const saved = localStorage.getItem(`progress-${'Node.js'}`);
    if (saved) {
  ...
  const handleSearch = (term) => {
    setSearchTerm(term);
    if (!term) {
      setFilteredQuestions(nodejsData[activeLevel]);
      return;
    }
    const filtered = nodejsData[activeLevel].filter(
      (item) =>
  ...
  const handleLevelChange = (level) => {
    ...
    setSearchTerm('');
    setFilteredQuestions(nodejsData[level]);
  };
  ...
  const toggleQuestion = (questionId) => {
    ...
    localStorage.setItem(`progress-${'Node.js'}`, JSON.stringify([...newCompleted]));
  };

  const totalQuestions =
    (nodejsData.basic?.length || 0) +
    (nodejsData.intermediate?.length || 0) +
    (nodejsData.advanced?.length || 0);
