...
export default function NextjsPage() {
  ...
  const [filteredQuestions, setFilteredQuestions] = useState(nextjsData[activeLevel]);
  const [completedQuestions, setCompletedQuestions] = useState(new Set());

  useEffect(() => {
    const saved = localStorage.getItem(`progress-${'Next.js'}`);
    if (saved) {
  ...
  const handleSearch = (term) => {
    setSearchTerm(term);
    if (!term) {
      setFilteredQuestions(nextjsData[activeLevel]);
      return;
    }
    const filtered = nextjsData[activeLevel].filter(
      (item) =>
  ...
  const handleLevelChange = (level) => {
    ...
    setSearchTerm('');
    setFilteredQuestions(nextjsData[level]);
  };
  ...
  const toggleQuestion = (questionId) => {
    ...
    localStorage.setItem(`progress-${'Next.js'}`, JSON.stringify([...newCompleted]));
  };

  const totalQuestions =
    (nextjsData.basic?.length || 0) +
    (nextjsData.intermediate?.length || 0) +
    (nextjsData.advanced?.length || 0);
