...
export default function JavascriptPage() {
  ...
  const [filteredQuestions, setFilteredQuestions] = useState(javascriptData[activeLevel]);
  const [completedQuestions, setCompletedQuestions] = useState(new Set());

  useEffect(() => {
    const saved = localStorage.getItem(`progress-${'JavaScript'}`);
    if (saved) {
  ...
  const handleSearch = (term) => {
    setSearchTerm(term);
    if (!term) {
      setFilteredQuestions(javascriptData[activeLevel]);
      return;
    }
    const filtered = javascriptData[activeLevel].filter(
      (item) =>
  ...
  const handleLevelChange = (level) => {
    ...
    setSearchTerm('');
    setFilteredQuestions(javascriptData[level]);
  };
  ...
  const toggleQuestion = (questionId) => {
    ...
    localStorage.setItem(`progress-${'JavaScript'}`, JSON.stringify([...newCompleted]));
  };

  const totalQuestions =
    (javascriptData.basic?.length || 0) +
    (javascriptData.intermediate?.length || 0) +
    (javascriptData.advanced?.length || 0);
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        >
          ...
          <h2 className="text-2xl md:text-3xl font-semibold text-gray-300 mb-4">
            {javascriptData.titleB}
          </h2>
          <p className="text-xl text-gray-400 max-w-2xl mx-auto">
            {javascriptData.description}
          </p>
        </motion.div>
